...
public class OrderController {
    ...
    @PostMapping()
    public ResponseEntity<?> create(@RequestBody NewOrderDTO orderDTO){
        try {
    ...
    public ResponseEntity<?> list(){
        try {
            List<OrderDTO> result = service.list();
            return ResponseEntity.ok().body(result);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e);
        }
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> update(@RequestBody OrderDTO orderDTO, @PathVariable("id") Long id){
        try {
